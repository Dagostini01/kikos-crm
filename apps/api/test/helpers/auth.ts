import type { FastifyInstance, InjectOptions } from 'fastify';

import { JoseEncrypter } from '@/cryptography/jose-encrypter.js';
import { env } from '@/env/index.js';

export async function createAccessToken(userId = 'test-user-id') {
  const encrypter = new JoseEncrypter(env.JWT_SECRET, env.JWT_ACCESS_EXPIRES_IN);

  return encrypter.encrypt({ sub: userId });
}

export async function authHeader(userId = 'test-user-id') {
  const accessToken = await createAccessToken(userId);

  return {
    authorization: `Bearer ${accessToken}`,
  };
}

export async function withAuth(app: FastifyInstance) {
  const headers = await authHeader();
  const originalInject = app.inject.bind(app);

  app.inject = ((opts: InjectOptions | string) => {
    if (typeof opts === 'string') {
      return originalInject(opts);
    }

    return originalInject({
      ...opts,
      headers: {
        ...headers,
        ...opts.headers,
      },
    });
  }) as typeof app.inject;

  return app;
}
